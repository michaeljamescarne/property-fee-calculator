...
-- SAFE VERSION: Only runs if the table exists

-- Check if table exists before adding columns
DO $$
BEGIN
  -- Check if firb_calculations table exists
  IF EXISTS (
    SELECT FROM information_schema.tables 
    WHERE table_schema = 'public' 
    AND table_name = 'firb_calculations'
  ) THEN
    ...
    RAISE NOTICE 'Successfully added property_classification and bedrooms columns to firb_calculations table';
  ELSE
    RAISE NOTICE 'Table firb_calculations does not exist. Skipping migration. Run 20250110_create_firb_calculations.sql first.';
  END IF;
END $$;
