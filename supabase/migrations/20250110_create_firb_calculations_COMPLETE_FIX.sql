-- Create enum types for FIRB calculations (checks each one individually)
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'citizenship_status') THEN
    ...
    RAISE NOTICE 'Created citizenship_status enum';
  ELSE
    RAISE NOTICE 'citizenship_status enum already exists';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'australian_state') THEN
    ...
    RAISE NOTICE 'Created australian_state enum';
  ELSE
    RAISE NOTICE 'australian_state enum already exists';
  END IF;
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'entity_type') THEN
    ...
    RAISE NOTICE 'Created entity_type enum';
  ELSE
    RAISE NOTICE 'entity_type enum already exists';
  END IF;
END $$;
