...
export function generateFIRBPDF(
  ...
  const addFooter = () => {
    ...
    doc.text(`Generated on ${formatDate()}`, pageWidth - margin, pageHeight - 10, {
      align: "right",
    });
  };
  ...
  const eligibilityText = eligibility.canPurchase
    ? eligibility.requiresFIRB
      ? "⚠ Eligible - FIRB Approval Required"
  ...
  doc.setFontSize(22);
  doc.setFont("helvetica", "bold");
  doc.text(formatCurrency(costs.totalInvestmentCost), pageWidth / 2, yPosition + 19, {
    align: "center",
  });
  ...
  const disclaimerText =
    "The estimates and outputs generated by the PropertyCosts.com.au calculator are for illustrative and hypothetical purposes only. " +
    "This information constitutes General Advice and does not take into account your personal objectives, financial situation, or needs. " +
    "We have not considered whether the information or the hypothetical scenarios generated are suitable for your specific circumstances. " +
    "The results must not be relied upon as professional legal, financial, or tax advice, and are not a substitute for consulting a qualified professional. " +
    "Before making any property investment, finance, or legal decisions, you should seek independent advice from a licensed financial planner, tax professional, or legal practitioner.";
