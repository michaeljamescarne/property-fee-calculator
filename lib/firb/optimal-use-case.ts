...
function calculateLongTermRental(
  ...
  return {
    ...
    riskLevel: "low",
    description:
      "Long-term rental provides stable, predictable income with minimal regulatory requirements.",
  };
}
...
function calculateShortStay(
  ...
  const managementFee = investmentInputs.selfManaged ? 0 : annualIncome * 0.2; // 20% for short-stay management
  ...
  let description =
    "Short-stay accommodation can provide higher income potential but requires more active management.";
  if (regulations && !regulations.shortStayPermitted) {
    description =
      "Short-stay accommodation is not permitted in this area. Long-term rental is the only option.";
...
export function calculateOptimalUseCase(
  ...
  if (regulations && !regulations.shortStayPermitted) {
    ...
    reasoning.push(
      `Short-stay provides ${Math.round((netDifference / longTerm.netIncome) * 100)}% higher net income.`
    );
    ...
    reasoning.push(
      "Consider a mixed strategy: short-stay during peak seasons, long-term otherwise."
    );
  } else {
    optimalUse = "longTerm";
...
export function getDefaultOccupancyRate(state: AustralianState): number {
  ...
  const occupancyRates: Record<AustralianState, number> = {
    NSW: 0.7, // Sydney, tourist areas
    VIC: 0.68, // Melbourne
    QLD: 0.75, // Gold Coast, Cairns (high tourism)
    WA: 0.65,
    SA: 0.6,
    TAS: 0.7, // Hobart, tourist areas
    ACT: 0.55,
    NT: 0.7, // Darwin, tourist areas
  };
