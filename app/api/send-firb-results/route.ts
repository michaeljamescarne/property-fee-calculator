...
export async function POST(request: NextRequest) {
  try {
      ...
      if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
        try {
          ...
          const calculationData = {
            ...
            eligibility_result: eligibility as EligibilityResult,
            cost_breakdown: costs as CostBreakdown,
            user_email: email,
            locale: locale || "en",
    ...
    try {
      ...
      const translations = await loadPDFTranslations(locale || "en");
      ...
      const pdfBlob =
        analytics && Object.keys(analytics).length > 0
          ? await generateEnhancedPDF(
              ...
              translations,
              "premium",
              { projectionChart: null, cashFlowChart: null, roiComparisonChart: null } // Charts not available in email context (generated client-side)
            )
