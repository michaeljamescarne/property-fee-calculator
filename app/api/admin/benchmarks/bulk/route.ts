...
function parseCSV(csvText: string): Partial<CSVRow>[] {
...
function validateRow(row: Partial<CSVRow>, index: number): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (
    row.is_active &&
    !["true", "false", "1", "0", "yes", "no"].includes(row.is_active.toLowerCase())
  ) {
...
function transformRow(row: Partial<CSVRow>): {
  ...
  try {
    ...
    for (let i = 0; i < transformedRows.length; i++) {
      ...
      try {
        ...
        let query = supabase.from("benchmark_data").select("id").eq("state", row.state);
        ...
        if (existing) {
          ...
          // Insert new record
          const { error: insertError } = await supabase.from("benchmark_data").insert(row as never);
