  ...
  return `
        <div class="mt-6 p-6 bg-gray-50 rounded-lg">
            ...
            <select onchange="handleVisaTypeChange(this.value)"
                ...
                ${Object.entries(WIZARD_VISA_TYPES)
                  .map(
                    ([key, visa]) => `
...
function renderStep2_PropertyType() {
  return `
        ...
        <div class="space-y-4">
            ${Object.entries(PROPERTY_TYPES_WIZARD)
              .map(([key, prop]) => {
...
function handleCitizenshipChange(value) {
  ...
  if (value === "australian") {
    let message =
      "As an Australian citizen, you can purchase any residential property without FIRB approval. You only pay standard stamp duty and fees.";
    let reason = "Australian citizens do not require FIRB approval";

    // Try to get message from FIRB module if available
    if (
      typeof window.FIRBEligibility !== "undefined" &&
      typeof window.FIRBEligibility.checkFIRBEligibility === "function"
    ) {
      try {
  ...
  if (value === "permanent") {
    let message =
      "As a permanent resident ordinarily resident in Australia, you can purchase residential property without FIRB approval.";
    let reason =
      "Permanent residents ordinarily resident in Australia generally do not require FIRB approval";
    let caveat =
      "Note: You must be ordinarily resident in Australia. If you spend most of your time overseas, FIRB approval may be required.";
...
function calculateEligibilityResult() {
  ...
  try {
    ...
    console.log(
      "[WIZARD DEBUG] window.FIRBEligibility.checkFIRBEligibility:",
      typeof window.FIRBEligibility?.checkFIRBEligibility
    );
    ...
    if (
      ...
      console.error(
        "[WIZARD] Available modules:",
        Object.keys(window).filter((key) => key.includes("FIRB"))
      );
...
function renderEligibilityResult() {
  ...
  if (eligible && canProceedToCalculator) {
    ...
    const documentsKey =
      citizenshipStatus === "temporary" ? "temporary_resident" : "foreign_national";
                            ...
                            ${
                              alternatives && alternatives.length > 0
                                ...
                                `
                                    ...
                                    .join("")
                                : getAlternativeProperties(citizenshipStatus, visaType)
                                    .map(
                                      (alt) => `
                                    <li class="flex items-start">
                                        <svg class="w-5 h-5 text-green-600 mr-3 flex-shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
