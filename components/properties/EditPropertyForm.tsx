...
export default function EditPropertyForm({ property, locale }: EditPropertyFormProps) {
  const t = useTranslations("Properties.editProperty");
  const router = useRouter();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>({});

  // Form state - initialize with existing property data
  const [formData, setFormData] = useState({
    property_name: property.property_name || "",
    property_address: property.property_address || "",
    property_state: property.property_state as AustralianState,
    property_type: property.property_type as PropertyType,
    property_classification: property.property_classification as "unit" | "house" | null,
    ...
    purchase_costs: (property.purchase_costs || 0).toString(),
    ...
    current_loan_balance: property.current_loan_balance?.toString() || "",
    interest_rate: property.interest_rate?.toString() || "",
    loan_term_years: property.loan_term_years?.toString() || "",
    loan_type: property.loan_type as "principalAndInterest" | "interestOnly" | null,
    is_rental: property.is_rental || false,
    weekly_rent: property.weekly_rent?.toString() || "",
    property_management_fee_percent: property.property_management_fee_percent?.toString() || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validate
      const validation = propertyUpdateSchema.safeParse({ ...submitData, id: property.id });
      ...
      if (!response.ok) {
        if (result.details) {
          ...
          setErrors({ submit: result.message || "Failed to update property" });
        }
        setIsSubmitting(false);
        return;
      }
